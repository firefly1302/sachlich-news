...
export default async function AlternativPage() {
  ...
  let articles = await getCachedFeed('alternativ');

  if (!articles) {
    // Cache miss - fetch fresh
    console.log('📰 Alternativ: Cache miss - fetching fresh data');
    const rawArticles = await fetchNewsByCategory('alternativ');
    ...
    // Cache for 15 min
    await setCachedFeed(articles, 'alternativ');
  }

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900">Alternative Medien</h2>
        <p className="text-gray-600 mt-2">Unabhängige und kritische Perspektiven - {articles.length} Artikel</p>
      </div>
