...
export const metadata: Metadata = {
  title: 'Zürich News | Sachlich.News',
  description: 'Lokale Nachrichten aus Zürich - sachlich aufbereitet ohne Drama und Sensationalismus.',
};

// ISR disabled - only manual refresh via button
export const revalidate = false;

export default async function ZuerichPage() {
  // Server-side data fetching with caching
  let articles = await getCachedFeed('zuerich');

  if (!articles) {
    // Cache miss - fetch fresh
    console.log('📍 Zürich: Cache miss - fetching fresh data');
    const rawArticles = await fetchNewsByCategory('zuerich');
    ...
    // Cache for 15 min
    await setCachedFeed(articles, 'zuerich');
  }

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900">Zürich</h2>
        <p className="text-gray-600 mt-2">Lokale Nachrichten aus Zürich - {articles.length} Artikel</p>
      </div>
